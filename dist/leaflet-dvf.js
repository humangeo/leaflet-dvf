...
L.ColorClassFunction = L.PiecewiseFunction.extend({
    ...
    initialize: function(classBreaks, colors, options) {
        ...
        for (var i = 0; i < classBreaks.length - 1; ++i) {
            var start = classBreaks[i], end = classBreaks[i + 1], startColor = colors[i], endColor = this.options.interpolate ? colors[Math.min(colors.length - 1, i + 1)] : colors[i];
    ...
    _createWordCloud: function(element, wordCloudOptions) {
        ...
        var draw = function(words, element) {
            return function(words) {
                ...
                d3.select(element).attr("transform", "translate(" + width / 2 + "," + height / 2 + ")").selectAll("text").data(words).enter().append("text").style("font-size", function(d) {
                    return d.size + "px";
                }).style("font-family", wordCloudOptions.fontFamily || "Impact").style("fill", function(d, i) {
                    return fill(i);
        ...
        d3.layout.cloud().size([ width, height ]).spiral("rectangular").timeInterval(Infinity).words(words).padding(5).rotate(rotation).font(wordCloudOptions.fontFamily || "Impact").fontSize(function(d) {
            return fontSize(d[countField]);
        }).on("end", draw(words, element)).start();
        return element;
    },
...
L.Control.Legend = L.Control.extend({
    ...
    onAdd: function(map) {
        ...
        L.DomEvent.addListener(container, "mouseover", this.toggleSize).addListener(container, "mouseout", this.toggleSize).addListener(container, "touchstart", this.toggleSize).addListener(container, "touchend", this.toggleSize).addListener(container, "mousedown", L.DomEvent.stopPropagation).addListener(container, "mousewheel", L.DomEvent.stopPropagation).addListener(container, "MozMousePixelScroll", L.DomEvent.preventDefault).addListener(container, "click", L.DomEvent.stopPropagation).addListener(container, "click", L.DomEvent.preventDefault);
        return container;
    },
    clear: function() {
