...
$(document).ready(function() {
	...
	var electionLayer = new L.ChoroplethDataLayer(electionResults,options);
	...
	var options2 = {
		...
		displayOptions: {
			'Romney': {
				displayName: 'Electoral Votes - Romney',
				fillColor: fillColorFunctionRomney,
				fillOpacity: opacityFunction
			}
		},
		includeLayer: function (record) {
			return record['Romney'] > 0;
		},
		layerOptions: {
			fillOpacity: 0.5,
			opacity: 1,
	...
	var electionLayer2 = new L.ChoroplethDataLayer(electionResults,options2);

	map.addLayer(electionLayer2);

	var currentElectionOptions = {
		recordsField: 'races.P',
		locationMode: L.LocationModes.STATE,
		codeField: '1',
		chartOptions: {
			'9[1=GOP].3': {
	...
	var electionChartLayer2012 = new L.PieChartDataLayer(electionResults, currentElectionOptions);

	map.addLayer(electionChartLayer2012);

	// From:  http://election2008.s3.amazonaws.com/votes/us-pres.json
	...
	var electionOptions = {
		...
		codeField: 'abbr',
		chartOptions: {
			'races.President..votes[id=1701].votes': {
				displayName: 'McCain',
				color: 'hsl(0,100%,25%)	',
				fillColor: 'hsl(0,80%,75%)',
				maxValue: 8000000
			},
			'races.President..votes[id=1918].votes': {
				displayName: 'Obama',
				color: 'hsl(240,100%,25%)',
	...
	var electionChartLayer2008 = new L.PieChartDataLayer(electionData,electionOptions);
